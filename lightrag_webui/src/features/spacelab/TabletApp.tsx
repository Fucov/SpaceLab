 ...
 *
 * 重构说明：
 * 左侧：宏观数据（MacroData）+ 实验舱状态（LabModuleInfo）+ 知识图谱 + 文档导入
...
function ActiveTaskTracker() {
  ...
  const blockedCount = trackers.filter((t) => t.status === 'blocked').length
  const errorCount = trackers.filter((t) => t.status === 'error').length

  return (
    <div className="flex flex-col gap-2">
      <h3 className="text-sm font-semibold text-gray-700 tracking-wide flex items-center gap-1.5">
        <Activity className="w-4 h-4" />
        活跃任务追踪
        <span className="ml-auto text-xs text-gray-400 font-normal">
          <span className="text-emerald-500">{runningCount} 运行</span>
          {blockedCount > 0 && <span className="text-amber-500 ml-1">· {blockedCount} 阻塞</span>}
          {errorCount > 0 && <span className="text-red-500 ml-1">· {errorCount} 异常</span>}
        </span>
      </h3>

      {trackers.length === 0 && (
        <div className="text-xs text-gray-400 py-2 text-center">暂无活跃任务</div>
      )}
        ...
        >
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-sm font-semibold text-gray-800">{tracker.moduleName}</div>
                <div className="text-xs text-gray-400">任务: {tracker.currentStep}</div>
              </div>
            </div>
          ...
          {tracker.status === 'blocked' && tracker.blockedReason && (
            <div className="mt-2 rounded border border-amber-200 bg-amber-50 px-2 py-1.5 text-xs text-amber-700 flex items-center gap-1.5">
          ...
          {tracker.status === 'error' && (
            <div className="mt-2 rounded border border-red-200 bg-red-50 px-2 py-1.5 text-xs text-red-600 flex items-center gap-1.5">
...
function ExecutionDraft() {
    ...
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <div className="text-base font-semibold text-gray-700">执行草稿箱</div>
            <div className="text-xs text-gray-400">暂无待执行草稿 · 请在下方对话框发送实验指令</div>
          </div>
        </div>
        ...
        {draftHistory.length > 0 && (
          <div className="space-y-2">
            <div className="text-xs text-gray-400 font-medium">最近已授权记录</div>
            {draftHistory.slice(0, 4).map((draft) => (
              <div key={draft.id} className="flex items-center gap-2 rounded border border-gray-100 bg-gray-50 px-3 py-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-gray-700 truncate">{draft.taskName}</div>
                  <div className="text-xs text-gray-400">{draft.targetModuleName} · {draft.device}</div>
                </div>
                <div className="text-xs text-gray-400 flex-shrink-0">{draft.authorizedAt}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {/* 提示 */}
        <div className="mt-4 rounded border border-dashed border-gray-200 bg-gray-50 p-3 text-center">
          <div className="text-sm text-gray-500 leading-relaxed">
            在下方对话框中输入实验指令<br />
            <span className="text-blue-500">示例：离心机实验，转速5000，时间10分钟</span>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className={`rounded-lg border bg-white p-4 ${currentDraft.authorized ? 'border-emerald-300' : 'border-blue-300'}`}>
      {/* 标题栏 */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className={`flex h-8 w-8 items-center justify-center rounded-full ${currentDraft.authorized ? 'bg-emerald-100' : 'bg-blue-100'}`}>
      ...
      {/* 任务基本信息 */}
      <div className="rounded border border-gray-100 bg-gray-50 p-3 mb-4">
        <div className="flex items-center gap-2">
          <span className="text-base font-semibold">{currentDraft.targetModuleName}</span>
          <ChevronRight className="w-4 h-4 text-gray-300" />
          <span className="text-base font-bold text-gray-800">{currentDraft.taskName}</span>
        </div>
      ...
      <div className="space-y-3 mb-4">
        ...
        {currentDraft.deviceParams.map((param) => (
          <div key={param.key} className="flex items-center gap-3">
            <label className="text-sm text-gray-500 w-20 flex-shrink-0">{param.key}</label>
            <div className="flex-1 flex items-center gap-2">
              {param.editable ? (
                <input
                  ...
                  className="flex-1 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-mono text-gray-800 outline-none focus:border-blue-400 focus:bg-white transition-colors"
                />
              ) : (
                <div className="flex-1 rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm font-mono text-gray-500">
                  {param.value}
                </div>
              )}
              <span className="text-sm text-gray-400 w-8">{param.unit}</span>
              {!param.editable && (
                <span className="text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">锁定</span>
              )}
            </div>
          </div>
        ))}
      ...
      {/* AI 原始解析 */}
      <div className="rounded border border-dashed border-gray-200 bg-gray-50 p-3 mb-4">
        <div className="text-xs text-gray-400 mb-1">AI 原始解析</div>
      ...
      {/* 授权执行结果 */}
      {authResult && (
        <div className={`rounded border px-4 py-3 mb-4 text-sm flex items-center gap-2 ${
          authResult.success
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="cursor-pointer flex items-center gap-2 rounded-lg border-2 border-red-300 bg-red-50 px-4 py-3 text-sm font-bold text-red-600 transition-all hover:bg-red-100 hover:border-red-400 disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        {!currentDraft.authorized && (
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 rounded-lg border-2 border-emerald-500 bg-emerald-600 px-4 py-3 text-base font-bold text-white transition-all hover:bg-emerald-700 disabled:opacity-60 cursor-pointer"
          >
            {authorizing ? (
              <>
        ...
        {currentDraft.authorized && (
          <div className="flex-1 flex items-center justify-center gap-2 rounded-lg border-2 border-emerald-300 bg-emerald-50 px-4 py-3 text-base font-bold text-emerald-600">
            <CheckCircle2 className="w-5 h-5" />
            已授权执行
          </div>
        )}
...
参数明细：
${draft.deviceParams.map((p) => `• ${p.key}：${p.value} ${p.unit}${p.editable ? '' : ' [锁定]'}`).join('\n')}
...
请尝试：离心机实验，转速3000，时间5分钟 / 启动生命科学舱`,
      timestamp: new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }),
    }
    addChatMessage(assistantMsg)
    setIsLoading(false)
  ...
  return (
    <div className="flex flex-col rounded-lg border border-gray-200 bg-white h-full">
      ...
      <div className="flex items-center gap-3 border-b border-gray-100 px-4 py-3">
        ...
        <div>
          <div className="text-base font-semibold text-gray-800">天宫智能助手</div>
          <div className="text-xs text-gray-400">支持实验指令解析 · 参数生成 · 执行授权</div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 min-h-0">
        {chatMessages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-300 py-12">
            <BotMessageSquare className="w-12 h-12 mb-4 text-gray-300" />
            <div className="text-base font-medium text-gray-500">天宫智能助手</div>
            <div className="text-sm mt-2 text-gray-400 text-center max-w-xs leading-relaxed">
            ...
            <div className="mt-6 flex flex-col gap-2 w-full max-w-xs">
              {[
                ...
                <button
                  ...
                  className="cursor-pointer rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm text-blue-600 hover:bg-blue-100 transition-colors text-left"
                >
                  {hint}
                </button>
              ))}
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="flex items-center gap-3 border-t border-gray-100 px-4 py-3">
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="发送实验指令，如：离心机实验..."
          disabled={isLoading}
          className="flex-1 rounded-lg border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-800 outline-none transition-colors placeholder:text-gray-300 focus:border-blue-400 focus:bg-white"
        />
        <button
          type="submit"
          disabled={isLoading || !input.trim()}
          className="cursor-pointer rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
        >
    ...
    <div className="fixed inset-0 flex flex-col overflow-hidden bg-[#f8fafc]">
      ...
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-gray-200 bg-white px-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-base font-semibold text-gray-700">天宫平板终端 · HITL 交互模式</span>
        </div>
      ...
      <div className="flex flex-1 min-h-0 gap-4 p-4 overflow-hidden">
        {/* 左侧 - 活跃任务追踪器 + 文档上传 */}
        <div className="w-[30%] min-w-[240px] flex flex-col gap-4 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
          <ActiveTaskTracker />
          <DocumentUpload />
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4 overflow-hidden">
          {/* 上：执行草稿箱 */}
          <div className="flex-shrink-0 overflow-y-auto max-h-[45%] scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
            <ExecutionDraft />
          </div>
