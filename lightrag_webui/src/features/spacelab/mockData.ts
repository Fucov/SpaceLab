...
export const scheduledTasks: ScheduledTask[] = [
  {
    ...
    priority: 'high',
    dagStage: 'DAG 第3阶段',
    scheduleHint: '正在执行，预计 12 分钟后释放机械臂',
    gates: {
      dependency: { status: 'passed', summary: '前驱已完成', predecessors: ['细胞复苏', '培养箱预热'], done: ['细胞复苏', '培养箱预热', '培养基配制'], satisfied: true },
      resource: { status: 'passed', summary: '资源已锁定', required: ['机械臂#02', 'CO₂培养箱'], active: ['机械臂#02'], conflict: false },
      safety: { status: 'passed', summary: '环境满足阈值', predicate: '温度37.2°C≤37.5°C，CO₂≤0.05%，功率余量>2kW', satisfied: true },
    },
  },
  {
    id: 'sched-02',
    ...
    priority: 'high',
    dagStage: '并行组 1',
    scheduleHint: '与液滴注入并行，已占用高速相机',
    gates: {
      dependency: { status: 'passed', summary: '清洗与预热完成', predecessors: ['毛细管清洗', '工质预热'], done: ['毛细管清洗', '工质预热'], satisfied: true },
      resource: { status: 'passed', summary: '高速相机已锁定', required: ['高速相机#01'], active: ['高速相机#01'], conflict: false },
      safety: { status: 'passed', summary: '振动与功率正常', predicate: '振动<0.02g，功率41%<70%，姿态稳定', satisfied: true },
    },
  },
  {
    id: 'sched-03',
    ...
    priority: 'medium',
    dagStage: '并行组 1',
    scheduleHint: '预计 20:26 开始',
    gates: {
      dependency: { status: 'passed', summary: '前驱满足', predecessors: ['腔体清洗', '工质预热'], done: ['腔体清洗', '工质预热'], satisfied: true },
      resource: { status: 'passed', summary: '注射泵空闲', required: ['微量注射泵#03'], active: ['高速相机#01'], conflict: false },
      safety: { status: 'passed', summary: '压力稳定', predicate: '压力101.1kPa，温度21.8°C，液位>15%', satisfied: true },
    },
  },
  {
    id: 'sched-04',
    ...
    priority: 'high',
    dagStage: 'DAG 第2阶段',
    scheduleHint: '安全门拒绝：培养箱温度超限历史未复核',
    gates: {
      dependency: { status: 'passed', summary: '样品定量完成', predecessors: ['样品裂解', '浓度测定'], done: ['样品裂解', '浓度测定'], satisfied: true },
      resource: { status: 'passed', summary: '结晶箱可用', required: ['结晶培养箱#02'], active: ['电泳仪'], conflict: false },
      safety: { status: 'rejected', summary: '温控风险未解除', predicate: '培养箱温度25.8°C>25.0°C，HEPA压差告警未闭环', satisfied: false },
    },
  },
  {
    id: 'sched-05',
    order: 5,
    moduleId: 'earth-observe',
    moduleName: '对地观测舱',
    taskName: '遥感载荷标定',
    status: 'waiting_resource',
    priority: 'medium',
    dagStage: '并行组 2',
    scheduleHint: '等待 LLM 推理单元A释放',
    gates: {
      dependency: { status: 'passed', summary: '轨道窗口已确认', predecessors: ['轨道窗口计算', '热控稳定'], done: ['轨道窗口计算', '热控稳定'], satisfied: true },
      resource: { status: 'waiting', summary: '推理单元冲突', required: ['LLM推理单元A', '高光谱仪'], active: ['LLM推理单元A'], conflict: true },
      safety: { status: 'passed', summary: '姿态满足观测', predicate: '姿态误差0.01°/s≤0.03°/s，功率余量>2kW', satisfied: true },
    },
  },
  {
    id: 'sched-06',
    ...
    priority: 'low',
    dagStage: 'DAG 第5阶段',
    scheduleHint: '等待热处理记录归档',
    gates: {
      dependency: { status: 'waiting', summary: '前驱未闭环', predecessors: ['铜模吸铸', '热处理退火', '样品归档'], done: ['铜模吸铸', '热处理退火'], satisfied: false },
      resource: { status: 'unchecked', summary: '依赖未满足，暂未检查', required: ['XRD分析仪', 'LLM推理单元B'], active: ['LLM推理单元B'], conflict: false },
      safety: { status: 'unchecked', summary: '依赖未满足，暂未检查', predicate: '样品温度≤28°C，冷却回路正常', satisfied: false },
    },
  },
  {
    id: 'sched-07',
    ...
    priority: 'medium',
    dagStage: 'DAG 第4阶段',
    scheduleHint: '预计 20:34 进入图像分析',
    gates: {
      dependency: { status: 'passed', summary: '图像采集完成', predecessors: ['点火触发', '高速成像'], done: ['点火触发', '高速成像', '舱内吹扫'], satisfied: true },
      resource: { status: 'passed', summary: '算力窗口已预留', required: ['LLM推理单元C', '图像缓存区'], active: ['LLM推理单元A', 'LLM推理单元B'], conflict: false },
      safety: { status: 'passed', summary: '燃烧舱已吹扫', predicate: 'O₂<5%，CO₂<0.06%，温度<35°C', satisfied: true },
    },
  },
  {
    id: 'sched-08',
    ...
    priority: 'medium',
    dagStage: '并行组 4',
    scheduleHint: '等待微重力诱导培养结束',
    gates: {
      dependency: { status: 'waiting', summary: '培养阶段未完成', predecessors: ['细胞接种', '微重力诱导培养'], done: ['细胞接种'], satisfied: false },
      resource: { status: 'unchecked', summary: '等待依赖后检查', required: ['荧光显微镜', 'LLM推理单元A'], active: ['机械臂#02'], conflict: false },
      safety: { status: 'unchecked', summary: '等待依赖后检查', predicate: '染色剂余量>20%，温度37±0.5°C', satisfied: false },
    },
  },
  {
    id: 'sched-09',
    ...
    priority: 'low',
    dagStage: 'DAG 第1阶段',
    scheduleHint: '已完成，释放观测窗口',
    gates: {
      dependency: { status: 'passed', summary: '无前驱', predecessors: [], done: [], satisfied: true },
      resource: { status: 'passed', summary: '资源已释放', required: ['轨道窗口计算器'], active: [], conflict: false },
      safety: { status: 'passed', summary: '安全闭环', predicate: '姿态稳定，功率余量>3kW', satisfied: true },
    },
  },
  {
    id: 'sched-10',
    ...
    priority: 'high',
    dagStage: 'DAG 第2阶段',
    scheduleHint: '等待电泳仪复位与安全复核',
    gates: {
      dependency: { status: 'passed', summary: '异常日志已生成', predecessors: ['电流保护触发', '断电记录'], done: ['电流保护触发', '断电记录'], satisfied: true },
      resource: { status: 'waiting', summary: '设备被锁定检修', required: ['电泳仪', '电源通道B'], active: ['电泳仪'], conflict: true },
      safety: { status: 'waiting', summary: '等待复核', predicate: '电流回路正常，舱内温度≤25°C', satisfied: false },
    },
  },
]
